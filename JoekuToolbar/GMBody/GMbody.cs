			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				if ( targeted is Mobile )
				{
					...
					else 
					{
                        ...
                        //string prefix = Commands.CommandSystem.Prefix;
						...
						if(from.AccessLevel >= AccessLevel.Counselor)
						{
							...
							for ( int i = 0; i < targ.Skills.Length; ++i )
								targ.Skills[i].Base = 999;
						}

						if(from.AccessLevel == AccessLevel.Counselor)
						{
							EquipItem( new GMSash() );
							EquipItem( new CounselorRobe() );
							EquipItem( new Boots( 3 ) );
						...
						if(from.AccessLevel >= AccessLevel.Administrator)
						{
							...
							EquipItem( new GMSash() );
							EquipItem( new GMPlateChest() );
							EquipItem( new GMPlateLegs() );
							EquipItem( new GMPlateArms() );
							EquipItem( new GMPlateGloves() );
							EquipItem( new GMPlateGorget() );
							EquipItem( new GMHelm() );
							EquipItem( new GMCloak() );
							EquipItem( new OwnerStaff() );
						}
